...
if GPS_PART1:
    ...
    PREDICTIVE_PATTERNS_QUERY = WITH + GPS_BANNER_PRED_PATTERNS + \
        ...
        buildSpatialFeatures(startSlash=32, endSlash=48, L4 = True, ASN = useASN)+\
        buildCrossJoinFeatures(slashes = [STEP_SIZE],ASN = useASN) +\
...
if not GPS_PART1:
    ...
    FINAL_SCANNING_PLAN=DATAPATH+"pred_scan_list"
    INAL_SCANNING_LIST_CSV = FINAL_SCANNING_PLAN + ".csv"
    df = run_bq_query(GET_PREDICTIONS_QUERY,BQ_RESOURCE_PROJECT,\
             BQ_DATASET, PRED_RESULTS_TABLE, SAVE=True)
    df.to_csv(INAL_SCANNING_LIST_CSV, index=False, header=True)
    print("Here is a snippet of the final scan list: ")
    print(df)
    sys.exit()
